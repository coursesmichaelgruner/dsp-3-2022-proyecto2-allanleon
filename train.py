...
def get_array(file_list:list[str],rows=40,cols=100):
    ...
    for i in range(len(file_list)):
        ...
        r,c = data.shape
        if r != rows or c != cols:
            data=np.pad(data,[(0,rows-r),(0,cols-c)],mode='constant',constant_values=0)
...
training_x,labels_train=get_array(training_list)
validatioin_x,labels_valid=get_array(validation_list)
